class HoldCircle {
    ...
    updateFill() {
        ...
        if (this.progress > 100) {
            clearInterval(this.intervalId);
            this.progress = 100; // Ensures the circle completes before callback
        } else {
            this.drawCircle();
        }
    }
...
const holdCircle = new HoldCircle({
    ...
    strokeWidth: 5,
    radius: 25,
    global: false, // Set to true for global, false for data-holdcircle elements
    easingFunction: (t) => t * (2 - t), // Example ease-out quadratic
    callback: function() { alert('Hold circle complete!'); }
});